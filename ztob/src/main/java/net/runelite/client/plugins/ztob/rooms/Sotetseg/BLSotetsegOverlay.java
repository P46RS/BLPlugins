...
public class BLSotetsegOverlay extends BLRoomOverlay
{
	@Inject
	...
	@Inject
	protected BLSotetsegOverlay(BLTheatreConfig config)
	{
		super(config);
	}
		...
		if (sotetseg.isSotetActive())
        {
			...
			if (config.sotetsetAttacks() || config.sotetsetAttacks1())
			{
				for (Projectile p : client.getProjectiles())
				{
					...
					if (id == BLSotetseg.SOTETSEG_MAGE_ORB && config.sotetsetAttacks())
					{
					...
					if (id == BLSotetseg.SOTETSEG_RANGE_ORB && config.sotetsetAttacks())
					{
					...
					if (id == BLSotetseg.SOTETSEG_BIG_AOE_ORB && config.sotetsetAttacks1())
					{
						Point txtPoint = new Point(point.getX(), point.getY());
						point = new Point(point.getX() - BLSotetseg.TACTICAL_NUKE_OVERHEAD.getWidth() / 2, point.getY() - 60);
						OverlayUtil.renderImageLocation(graphics, point, BLSotetseg.TACTICAL_NUKE_OVERHEAD);
