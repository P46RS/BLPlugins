...
public class BLXarpusOverlay extends BLRoomOverlay
{
	@Inject
	private BLXarpus xarpus;

	@Inject
	protected BLXarpusOverlay(BLTheatreConfig config)
	{
		super(config);
	}
	...
	public Dimension render(Graphics2D graphics)
	{
		...
		if (xarpus.isXarpusActive())
        {
            ...
            boolean showp2 = config.xarpusTick1() && BLXarpus.P2_IDS.contains(boss.getId());
            boolean p3exception = xarpus.isHM() && xarpus.isXarpusStare() && xarpus.isP3Active();
			boolean showp3 = config.xarpusTick2() && BLXarpus.P3_IDS.contains(boss.getId()) && !p3exception;
            if (showp2 || showp3)
            {
            ...
            if (BLXarpus.P1_IDS.contains(boss.getId()))
            {
