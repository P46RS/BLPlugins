...
@Slf4j
public class BLSotetseg extends BLRoom
{
	...
	@Inject
	private BLSotetsegOverlay BLSotetsegOverlay;
	@Inject
	...
	@Inject
	protected BLSotetseg(BLTheatrePlugin plugin, BLTheatreConfig config)
	{
		super(plugin, config);
	}
	...
	public void load()
	{
		overlayManager.add(BLSotetsegOverlay);
	}

	@Override
	public void unload()
	{
		overlayManager.remove(BLSotetsegOverlay);
	}
	...
	WorldPoint worldPointFromMazePoint(Point mazePoint)
	{
		if (overWorldRegionID == -1 && client.getLocalPlayer() != null)
		{
			return WorldPoint.fromRegion(
				client.getLocalPlayer().getWorldLocation().getRegionID(), mazePoint.getX() + BLSotetseg.getSwMazeSquareOverWorld().getX(),
				mazePoint.getY() + BLSotetseg.getSwMazeSquareOverWorld().getY(), 0);
		}
		return WorldPoint.fromRegion(
			overWorldRegionID, mazePoint.getX() + BLSotetseg.getSwMazeSquareOverWorld().getX(),
			mazePoint.getY() + BLSotetseg.getSwMazeSquareOverWorld().getY(), 0);
	}
